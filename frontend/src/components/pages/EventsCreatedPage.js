...
const EventsCreatedPage = () => {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                const response = await axios.get(`https://api-gateway-xwjwz3lfdq-ez.a.run.app/api/event/host/${userInfo.id}`, {
                    headers: {
    ...
    return (
        <div>
            <Box p={{base: '4', md: '6'}} pt={{base: '6', md: '8'}} pl={{base: '4', md: '28'}}>
                <Heading fontWeight='bold' fontSize={{base: 'xl', sm: '2xl', md: '6xl'}} lineHeight={'110%'}>Events
                    Created</Heading>
                <Text color={'gray.500'} mt='2'>
                    Here are all the events you have created. You can view the details of each event, modify them and
